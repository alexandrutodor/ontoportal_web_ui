...
class WorkflowsController < ApplicationController
  ...
  SECTIONS = %w[overview steps software_environment related_resources papers_code reproducibility].freeze
  FACETS = %i[family task domain engine software language license environment access maintenance group project].freeze
  ...
  def index
    ...
    @group_options = facet_options(:group, catalogue).uniq.sort_by(&:downcase)
    @project_options = facet_options(:project, catalogue).uniq.sort_by(&:downcase)
  ...
  def show
    @workflow = scoped_catalogue.find { |workflow| workflow[:catalogue_id].to_s == params[:id].to_s }
  ...
  def api_show
    workflow = scoped_catalogue.find { |w| w[:catalogue_id].to_s == params[:id].to_s }
    return render json: { error: 'Workflow not found', id: params[:id] }, status: :not_found unless workflow

    fair = fair_score_for(workflow)
    render json: api_workflow_payload(workflow, fair)
  end

  private

  def api_workflows_payload(workflows)
    {
      total: workflows.length,
      page: 1,
      page_count: 1,
      collection: workflows.map { |w| api_workflow_payload(w, self.class.fair_score_for(w)) }
    }
  end

  def api_workflow_payload(workflow, fair)
    workflow.merge(
      fair_readiness: {
      ...
      links: {
        ui: "/workflows/#{workflow[:catalogue_id]}",
        api: "/api/v1/workflows/#{workflow[:catalogue_id]}"
      }
    )
  end
  ...
  def facet_options(facet, collection = scoped_catalogue)
    collection.flat_map { |workflow| facet_values(workflow, facet) }.compact.uniq.sort_by(&:downcase)
  end

  def selected_values(facet, collection = scoped_catalogue)
    allowed = if facet == :family
                TAXONOMY_FAMILIES
              else
                facet_options(facet, collection)
              end
  ...
  def matches_filters?(workflow, collection = scoped_catalogue)
  ...
  def facet_values(workflow, facet)
    ...
    when :group then Array(workflow[:groups]) + Array(workflow[:slices])
    when :project then Array(workflow[:projects])
    else []
    end
  end
end
